...
class CheckoutV2Test < Test::Unit::TestCase
  ...
  def test_successful_purchase_using_visa_network_token
  ...
  def test_successful_purchase_using_mastercard_network_token
    network_token = network_tokenization_credit_card(
      '4242424242424242',
      { source: :network_token, brand: 'mastercard' }
    )
    response = stub_comms do
      @gateway.purchase(@amount, network_token)
    end.check_request do |_endpoint, data, _headers|
  ...
  def test_successful_purchase_using_applepay_network_token
    network_token = network_tokenization_credit_card(
      '4242424242424242',
      { source: :apple_pay, payment_cryptogram: 'AgAAAAAAAIR8CQrXcIhbQAAAAAA' }
    )
    response = stub_comms do
      @gateway.purchase(@amount, network_token)
    end.check_request do |_endpoint, data, _headers|
  ...
  def test_successful_purchase_using_googlepay_network_token
    network_token = network_tokenization_credit_card(
      '4242424242424242',
      { source: :google_pay, payment_cryptogram: 'AgAAAAAAAIR8CQrXcIhbQAAAAAA' }
    )
    response = stub_comms do
      @gateway.purchase(@amount, network_token)
    end.check_request do |_endpoint, data, _headers|
